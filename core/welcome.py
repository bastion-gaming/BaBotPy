...
async def memberjoin(member, channel):
    if member.guild.id == idBASTION:
        ...
        ID = member.id
        req = requests.get('http://{ip}/users/playerid/{discord_id}'.format(ip=ge.API_IP, discord_id=ID), verify=False).json()
        if req['error'] == 404:
...
async def memberremove(member, channel):
    ...
    if member.guild.id == idBASTION:
        ...
        requests.put('http://{ip}/users/xp/{player_id}/{nb}'.format(ip=ge.API_IP, player_id=PlayerID, nb=-balXP))
        requests.put('http://{ip}/users/level/{player_id}/{nb}'.format(ip=ge.API_IP, player_id=PlayerID, nb=-balLvl))
