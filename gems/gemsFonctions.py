...
def recette(ctx):
    ...
    d_recette += "▬▬▬▬▬▬▬▬▬▬▬▬▬\n**Forge**\n"
    for c in objetOutil:
        for recette in objetRecette:
            if c.type == "forge":
                if c.nom == recette.nom:
                    d_recette += "<:gem_{0}:{1}>`{0}`: ".format(c.nom, get_idmoji(c.nom))
                    if recette.nb1 > 0:
                        d_recette += "{0} <:gem_{1}:{2}>`{1}` ".format(recette.nb1, recette.item1, get_idmoji(recette.item1))
                    if recette.nb2 > 0:
                        d_recette += "et {0} <:gem_{1}:{2}>`{1}` ".format(recette.nb2, recette.item2, get_idmoji(recette.item2))
                    if recette.nb3 > 0:
                        d_recette += "et {0} <:gem_{1}:{2}>`{1}` ".format(recette.nb3, recette.item3, get_idmoji(recette.item3))
                    if recette.nb4 > 0:
                        d_recette += "et {0} <:gem_{1}:{2}>`{1}` ".format(recette.nb4, recette.item4, get_idmoji(recette.item4))
                    d_recette += "\n"
