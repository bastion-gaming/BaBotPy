...
class GemsEvent(commands.Cog):
	...
	async def cooked(self, ctx):
		...
		ID = ctx.author.id
		if DB.spam(ID,GF.couldown_l, "cooked"):
			if DB.nbElements(ID, "cooked", "furnace_1") == 0:
				if DB.nbElements(ID, "inventory", "pumpkin") >= 12:
					DB.add(ID, "cooked", "furnace_1", t.time())
					DB.add(ID, "inventory", "pumpkin", -12)
				...
				CookedTime = DB.nbElements(ID, "cooked", "furnace_1")
				InstantTime = t.time()
				time = CookedTime - (InstantTime-GF.couldown_2h)
				if time <= 0:
					DB.add(ID, "inventory", "oak", 1)
					DB.add(ID, "cooked", "furnace_1", -1*CookedTime)
